...
std::vector<std::pair<std::string, std::string>> rm_vec = 
    {{"not","00"}, {"inc","01"},{"dec","10"},{"mov","11"}};
...
void one_op_inst(std::vector<std::string> inst_vec, std::ofstream &FileWrite) {
 std::string instruction;
 std::string rm_padding;
 std::string final_val;
 if(inst_vec.at(0) == "inc" || inst_vec.at(0) == "dec") {
  instruction = "10";
  ...
  instruction.append(rm_padding);
  std::string reg = registers(inst_vec.at(1));
...
void bnz_one_op_inst(std::vector<std::string> inst_vec, 
    std::map<std::string, int> label_map, std::ofstream &FileWrite) {
    if(inst_vec.at(0) == "inc" || inst_vec.at(0) == "dec"){
     one_op_inst(inst_vec, FileWrite);
     return;
    }
    std::string instruction;
    std::string final_val;
...
int main(int argc, char** argv) {
    ...
    for(auto &line2: lines){
        ...
        if(single_lines.size() == 2) {
            bnz_one_op_inst(single_lines, label_map, FileWrite);
